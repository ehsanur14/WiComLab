...
M_list = [2 4 8 16 32 64 128 256 512 1024];
label_list = 0:9;

num_rows = numel(M_list) * numel(SNR_dB) * samples_per_snr;
num_features = 1 + 2*symb + 1;

logn_mpsk_dataset = zeros(num_rows, num_features, 'single');
gg_mpsk_dataset = zeros(num_rows, num_features, 'single');
...
for iM = 1:numel(M_list)
    ...
    for iSNR = 1:numel(SNR_dB)
        ...
        for sample_idx = 1:samples_per_snr
            ...
            logn_mpsk_dataset(row_idx,:) = single([SNR_dB(iSNR), ...
                real(rx_iq_log), imag(rx_iq_log), label]);
            gg_mpsk_dataset(row_idx,:) = single([SNR_dB(iSNR), ...
                real(rx_iq_gg), imag(rx_iq_gg), label]);

            row_idx = row_idx + 1;
        end

        fprintf('M=%4d PSK | label=%d | SNR=%3d dB | %d samples\n', ...
            M, label, SNR_dB(iSNR), samples_per_snr);
    end
end

save('logn_mpsk_dataset.mat', 'logn_mpsk_dataset', 'column_names', ...
    'M_list', 'label_list', 'SNR_dB', 'samples_per_snr', 'symb', '-v7.3');

save('gg_mpsk_dataset.mat', 'gg_mpsk_dataset', 'column_names', ...
    'M_list', 'label_list', 'SNR_dB', 'samples_per_snr', 'symb', '-v7.3');

%% ================= FUNCTIONS =================
function [Inphase, Quadrature] = Basebandmodulation(M, symb)
X = randi([0, M-1], symb, 1);
Y = pskmod(X, M, pi/4, 'gray');
Inphase = real(Y);
Quadrature = imag(Y);
end

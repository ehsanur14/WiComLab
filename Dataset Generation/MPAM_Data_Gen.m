...
logn_mpam_dataset = zeros(num_rows, num_features, 'single');
gg_mpam_dataset = zeros(num_rows, num_features, 'single');
...
for iM = 1:numel(M_list)
    ...
    for iSNR = 1:numel(SNR_dB)
        ...
        for sample_idx = 1:samples_per_snr
            ...
            logn_mpam_dataset(row_idx,:) = single([SNR_dB(iSNR), ...
                real(rx_iq_log), imag(rx_iq_log), label]);
            gg_mpam_dataset(row_idx,:) = single([SNR_dB(iSNR), ...
                real(rx_iq_gg), imag(rx_iq_gg), label]);

            row_idx = row_idx + 1;
        end

        fprintf('M=%4d PAM | label=%d | SNR=%3d dB | %d samples\n', ...
            M, label, SNR_dB(iSNR), samples_per_snr);
    end
end

save('logn_mpam_dataset.mat', 'logn_mpam_dataset', 'column_names', ...
    'M_list', 'label_list', 'SNR_dB', 'samples_per_snr', 'symb', '-v7.3');

save('gg_mpam_dataset.mat', 'gg_mpam_dataset', 'column_names', ...
    'M_list', 'label_list', 'SNR_dB', 'samples_per_snr', 'symb', '-v7.3');

%% ================= FUNCTIONS =================
function [Inphase, Quadrature] = Basebandmodulation(M, symb)
X = randi([0, M-1], symb, 1);
Y = pammod(X, M, 0, 'gray') ./ sqrt((M^2 - 1)/3);
Inphase = real(Y);
Quadrature = zeros(size(Y));
end
